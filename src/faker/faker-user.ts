    ...
    for (let i = 0; i < count; i++) {
      ...
      const user = await prisma.user.create({
        data: {
          ...
          isAdmin: faker.datatype.boolean(0),
          contacts: {
            create: generateFakeContacts(faker.number.int({ min: 1, max: 5 }))
          }
        }
      });
...
function generateFakeContacts(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    contacts.push({
      ...
      ofcNo: faker.number.int() ? faker.phone.number().slice(0, 20) : undefined,
      nationalId: faker.number.int({ min: 100000, max: 999999 }),
      addresses: {
        create: generateFakeAddresses(faker.number.int({ min: 1, max: 3 }))
      }
    });
  }
